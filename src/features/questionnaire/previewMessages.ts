...
const messagesByLocale: Record<AppLocale, PreviewMessages> = {
  'zh-Hant': {
    ...
    hardNoDialogDescription: '集中列出這份檔案中標示為「絕對禁止」的分類與細項。這份列表仍不能替代當下的確認與溝通。',
  ...
  'zh-Hans': {
    ...
    hardNoDialogDescription: '集中列出这份档案中标示为“绝对禁止”的分类与细项。这份列表仍不能替代当下的确认与沟通。',
  ...
  ja: {
    ...
    hardNoDialogDescription: 'このファイルで「絶対に不可」とされたカテゴリーと項目をまとめています。この一覧も、その時々の確認や対話に代わるものではありません。',
  ...
  en: {
    ...
    hardNoDialogDescription: 'This gathers every category and item marked “absolutely prohibited” in this file. It still cannot replace a current check-in and conversation.',
