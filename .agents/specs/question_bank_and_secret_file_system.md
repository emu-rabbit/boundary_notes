...
10. 使用者從結果頁點擊分類大按鈕時，入口判斷只依該分類目前方向的細項作答進度，不受分類層是否有題目或是否已作答影響。細項全未作答或全數作答時，不詢問並直接顯示本分類全部題目；只要細項處於部分作答狀態，就必須先詢問要「僅顯示未作答的題目」或「顯示本分類所有題目」。前者只顯示尚未填答的細項；後者對一般分類包含分類題本身與全部細項，合計為 N+1 題，`其他` 則顯示其全部 N 題細項。
