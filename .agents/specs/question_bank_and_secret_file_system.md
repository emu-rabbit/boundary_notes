...
- 喜好排行榜：使用者在主導側與配合側各自完全手動選擇最多 5 個項目，顯示在該方向的結果頁作為 spotlight，兩個方向不得共用候選選取狀態或儲存池。可選候選包含分類本身與細項，但僅限同方向中已作答且喜好為 `like` 或 `love` 的項目；未作答項目不可被選擇。選擇視窗以分類分組，每個分類只出現一次；同一分類內固定先顯示 `love`，再顯示 `like`。每個方向的五格只能點擊已登記的格子或最前方第一個空格；刪除後同方向池的後方項目依序前移。排行榜是使用者自行整理偏好，不得呈現為社交排行或鼓勵嘗試清單。
