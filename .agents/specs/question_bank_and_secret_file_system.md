...
建立分享連結前，確認 dialog 必須清楚說明持有完整連結者可讀、雲端快照長期保存且一般使用者目前無法自行修改或刪除，並提供以新分頁開啟的使用條款與隱私權政策。不得用預先勾選或模糊按鈕代替知情行為；使用者按下「了解並建立分享連結」（各語系等義文案）即表示已閱讀兩份法律文件，並明確同意上傳該份可能含有敏感內容的快照，不另加 checkbox。
