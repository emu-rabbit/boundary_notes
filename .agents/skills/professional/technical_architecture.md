...
- **同頁面重複快照防護**：成功建立雲端快照後，結果編輯頁只在目前 component 生命週期的記憶體中保留該檔案的分享內容 key 與既有 share URL。相同 `fileId`、名稱、題庫版本、scope、answers 與 spotlight 再次要求上傳時，不呼叫 Function，也不建立重複快照，直接引導至既有雲端檔案；內容實際改變後才允許建立新版本。不得為此建立跨使用者的 server-side 內容查詢或把敏感作答 hash 寫入公開資料。
