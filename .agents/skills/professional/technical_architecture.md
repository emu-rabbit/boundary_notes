...
- **production 人工 audit 身分**：本機人工稽核使用 `firestore-auditor@boundary-notes-prod.iam.gserviceaccount.com`，只授予 production project 的 `roles/datastore.viewer`，由操作者透過 Google Cloud ADC impersonation 取得短效憑證，不建立或保存 Service Account JSON key。版本控制內的工具固定放在 `ops/firestore-audit/`，只讀取總量與防濫用分析必要欄位；預設不得輸出完整 `profileName`，也不得讀取或輸出 answers、notes、Spotlight 或完整匿名 hash。若未來加入 Cloud Logging／Monitoring 檢查，必須另外評估並明確授予唯讀角色，不得順勢擴張 Firestore 寫入、Secret Manager 或部署權限。
